...
$(function() {
	...
	$(document).on("click", "#submitOccupy", function() {
		...
		var occupyName = ["【" + suiteType + " " + guidePrice + "】" + companyName, contactor, mobile]
			.join(",");
		var loading = weui.loading("loading");
		$.get("https://4001.cn/api/tenant/bookRepo/occupy?calleeNumber=" + calleeNumber +
			"&companyName=" + occupyName,
			function(res) {
				loading.hide();
				if (res.code === 0) {
	...
	$(document).on("click", "#submitMobile", function() {
		...
		const openId = companyName + "," + contactor + "," + mobile;
		localStorage.setItem("openId",openId);
		var loading = weui.loading("loading");
		$.get("https://4001.cn/api/tenant/bookRepo/occupy?calleeNumber=" + calleeNumber +
			"&companyName=" + openId).then((res) => {
			location.href = "./open_step_1.html?calleeNumber=" + calleeNumber +
				"&guidePrice=" +
				guidePrice + "&chargeCost=" + chargeCost;
