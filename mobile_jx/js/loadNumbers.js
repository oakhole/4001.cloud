...
function getSuiteNumberListFromPrice(pageNum, pageSize) {
	var tplSuiteNumberList = Handlebars.compile($("#suiteNumberList").html());
	$.get("https://400cha.cn/tenant/book/findBookListFromTag?pageNum=" + pageNum + "&pageSize=" + pageSize + "&memo=" +
		price,
		function(res) {
			if (res.code === 0) {
...
function openOccupyPage(calleeNumber, guidePrice) {
	...
	$.get("https://400cha.cn/tenant/book/isBooked?calleeNumber=" + calleeNumber, function(res) {
		if (res.code === 0) {
			// 跳转到预占表单
			location.href = "./occupyForm.html?calleeNumber=" + calleeNumber + "&guidePrice=" +
				selectedSuitePrice + "&suiteType=" + selectedSuiteType;
		} else {
