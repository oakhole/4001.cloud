...
var numberOperateFormatter = function(value, row, index) {
	...
	return '<a target="_blank" href="https://kt.4001.cn?uid=' + row.certification.certifyId + "&number=" + row
		.calleeNumber + "&openId=" + openId + '">重新提交</a>';
};
...
function submitNumber() {
	const submitType = $("#submitType").val();
	...
	if (compamyName == "" || contactPhone == "" || contact == "") {
		...
		if (submitType == "search") {
			dashangToggle();
			// 刷新已预占号码数据
			$.get("https://400cha.cn/tenant/bookRepo/occupied?calleeNumber=&companyName=" + openId, function(
				res) {
